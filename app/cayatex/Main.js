...
{
	if (region.cs.bx === region.dD.bx)
	{
		return 'on line ' + region.cs.bx;
	}
	return 'on lines ' + region.cs.bx + ' through ' + region.dD.bx;
}
...
function _VirtualDom_makeCallback(eventNode, initialHandler)
{
	function callback(event)
	{
		...
		var currentEventNode = (
			...
			(tag == 2 ? value.b : tag == 3 && value.c4) && event.preventDefault(),
			eventNode
		);
		var tagger;
		var i;
...
var _Browser_document = _Debugger_document || F4(function(impl, flagDecoder, debugMetadata, args)
{
	return _Platform_initialize(
		...
		impl.hT,
		function(sendToApp, initialModel) {
			var divertHrefToApp = impl.c9 && impl.c9(sendToApp)
			var view = impl.ik;
...
{
	var onUrlChange = impl.ha;
	var onUrlRequest = impl.hb;
	var key = function() { key.a(onUrlChange(_Browser_getUrl())); };

	return _Browser_document({
		c9: function(sendToApp)
		{
			key.a = sendToApp;
		...
		gF: function(flags)
		{
			return A3(impl.gF, flags, _Browser_getUrl(), key);
		},
		ik: impl.ik,
		ij: impl.ij,
		hT: impl.hT
...
function _Browser_visibilityInfo()
{
	return (typeof _VirtualDom_doc.hidden !== 'undefined')
		? { gv: 'hidden', fH: 'visibilitychange' }
		:
	(typeof _VirtualDom_doc.mozHidden !== 'undefined')
		? { gv: 'mozHidden', fH: 'mozvisibilitychange' }
		:
	(typeof _VirtualDom_doc.msHidden !== 'undefined')
		? { gv: 'msHidden', fH: 'msvisibilitychange' }
		:
	(typeof _VirtualDom_doc.webkitHidden !== 'undefined')
		? { gv: 'webkitHidden', fH: 'webkitvisibilitychange' }
		: { gv: 'hidden', fH: 'visibilitychange' };
}
...
function _Browser_getViewport()
{
	return {
		...
		e1: {
			e5: _Browser_window.pageXOffset,
			e6: _Browser_window.pageYOffset,
			il: _Browser_doc.documentElement.clientWidth,
...
function _Browser_getScene()
{
	...
	return {
		il: Math.max(body.scrollWidth, body.offsetWidth, elem.scrollWidth, elem.offsetWidth, elem.clientWidth),
		...
		while (true) {
			if (fromIndex < 0) {
				return A2(
					...
					false,
					{z: nodeList, s: (len / $elm$core$Array$branchFactor) | 0, w: tail});
			} else {
...
var $elm$url$Url$Url = F6(
	function (protocol, host, port_, path, query, fragment) {
		return {dJ: fragment, dU: host, ep: path, es: port_, eA: protocol, eB: query};
	});
...
var $elm$parser$Parser$Advanced$DeadEnd = F4(
	function (row, col, problem, contextStack) {
		return {fO: col, fU: contextStack, hr: problem, eJ: row};
	});
...
var $elm$parser$Parser$Advanced$fromState = F2(
	function (s, x) {
		return A2(
			...
			A4($elm$parser$Parser$Advanced$DeadEnd, s.eJ, s.fO, x, s.h));
	});
var $elm$parser$Parser$Advanced$end = function (x) {
	return function (s) {
		return _Utils_eq(
			$elm$core$String$length(s.hM),
			s.as) ? A3($elm$parser$Parser$Advanced$Good, false, 0, s) : A2(
...
var $elm$parser$Parser$Advanced$Located = F3(
	function (row, col, context) {
		return {fO: col, h: context, eJ: row};
	});
var $elm$parser$Parser$Advanced$changeContext = F2(
	function (newContext, s) {
		return {fO: s.fO, h: newContext, j: s.j, as: s.as, eJ: s.eJ, hM: s.hM};
	});
		...
		return function (s0) {
			var _v1 = parse(
				A2(
					...
					A2(
						$elm$core$List$cons,
						A3($elm$parser$Parser$Advanced$Located, s0.eJ, s0.fO, context),
						s0.h),
					s0));
			if (!_v1.$) {
				var p = _v1.a;
...
var $elm$parser$Parser$Advanced$token = function (_v0) {
	...
	return function (s) {
		var _v1 = A5($elm$parser$Parser$Advanced$isSubString, str, s.as, s.eJ, s.fO, s.hM);
		var newOffset = _v1.a;
		var newRow = _v1.b;
		var newCol = _v1.c;
		return _Utils_eq(newOffset, -1) ? A2(
			...
			progress,
			0,
			{fO: newCol, h: s.h, j: s.j, as: newOffset, eJ: newRow, hM: s.hM});
	};
};
...
var $elm$parser$Parser$Advanced$chompWhileHelp = F5(
	function (isGood, offset, row, col, s0) {
		chompWhileHelp:
		while (true) {
			var newOffset = A3($elm$parser$Parser$Advanced$isSubChar, isGood, offset, s0.hM);
			if (_Utils_eq(newOffset, -1)) {
				return A3(
					$elm$parser$Parser$Advanced$Good,
					_Utils_cmp(s0.as, offset) < 0,
					0,
					{fO: col, h: s0.h, j: s0.j, as: offset, eJ: row, hM: s0.hM});
			} else {
...
var $elm$parser$Parser$Advanced$chompWhile = function (isGood) {
	return function (s) {
		return A5($elm$parser$Parser$Advanced$chompWhileHelp, isGood, s.as, s.eJ, s.fO, s);
	};
};
...
var $elm$parser$Parser$Advanced$getOffset = function (s) {
	return A3($elm$parser$Parser$Advanced$Good, false, s.as, s);
};
var $elm$parser$Parser$Advanced$getSource = function (s) {
	return A3($elm$parser$Parser$Advanced$Good, false, s.hM, s);
};
...
var $author$project$Parser$Metadata$init = F4(
	function (generation, lineNumber, start, finish) {
		return {ft: lineNumber, bu: generation, a3: '', gT: finish - start, as: start};
	});
...
var $elm$parser$Parser$Advanced$chompIf = F2(
	function (isGood, expecting) {
		return function (s) {
			var newOffset = A3($elm$parser$Parser$Advanced$isSubChar, isGood, s.as, s.hM);
			return _Utils_eq(newOffset, -1) ? A2(
				$elm$parser$Parser$Advanced$Bad,
				false,
				...
				true,
				0,
				{fO: 1, h: s.h, j: s.j, as: s.as + 1, eJ: s.eJ + 1, hM: s.hM}) : A3(
				$elm$parser$Parser$Advanced$Good,
				true,
				0,
				{fO: s.fO + 1, h: s.h, j: s.j, as: newOffset, eJ: s.eJ, hM: s.hM}));
		};
	});
...
var $author$project$Parser$RawString$rawStringHelp = F2(
	function (hashes, state) {
		return (_Utils_cmp(state.aa, hashes) > -1) ? $elm$parser$Parser$Advanced$succeed(
...
var $elm$parser$Parser$Advanced$run = F2(
	function (_v0, src) {
		var parse = _v0;
		var _v1 = parse(
			{fO: 1, h: _List_Nil, j: 1, as: 0, eJ: 1, hM: src});
		if (!_v1.$) {
			var value = _v1.b;
...
var $author$project$Main$init = function (flags) {
	return _Utils_Tuple2(
		{
			C: 0,
			ai: $author$project$Main$initialText,
			a6: 1,
			aN: $author$project$Render$String$renderString($author$project$Main$initialText)
		},
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							C: model.C + 1,
							ai: str,
							aN: $author$project$Render$String$renderString(str)
						}),
					$elm$core$Platform$Cmd$none);
			case 5:
				var mode = msg.a;
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							C: model.C + 1,
							ai: '',
							aN: $author$project$Render$String$renderString('')
						}),
					$elm$core$Platform$Cmd$none);
			case 3:
				return _Utils_Tuple2(
					_Utils_update(
						...
						{
							C: model.C + 1,
							ai: $author$project$Data$text,
							aN: $author$project$Render$String$renderString($author$project$Data$text)
						}),
					$elm$core$Platform$Cmd$none);
			case 4:
				return _Utils_Tuple2(
					_Utils_update(
						...
						{
							C: model.C + 1,
							ai: $author$project$Data$test,
							aN: $author$project$Render$String$renderString($author$project$Data$test)
						}),
					$elm$core$Platform$Cmd$none);
			default:
				return _Utils_Tuple2(model, $elm$core$Platform$Cmd$none);
...
var $mdgriffith$elm_ui$Internal$Style$classes = {e8: 'a', cy: 'atv', fb: 'ab', fc: 'cx', fd: 'cy', fe: 'acb', ff: 'accx', fg: 'accy', fh: 'acr', dk: 'al', dl: 'ar', fi: 'at', cz: 'ah', cA: 'av', fm: 's', fp: 'bh', fq: 'b', fw: 'w7', fy: 'bd', fz: 'bdt', bU: 'bn', fA: 'bs', bW: 'cpe', fL: 'cp', fM: 'cpx', fN: 'cpy', u: 'c', bY: 'ctr', bZ: 'cb', b_: 'ccx', aw: 'ccy', bp: 'cl', b$: 'cr', fT: 'ct', fZ: 'cptr', f_: 'ctxt', go: 'fcs', dH: 'focus-within', gq: 'fs', gs: 'g', cU: 'hbh', cW: 'hc', dR: 'he', cX: 'hf', dS: 'hfp', gx: 'hv', gA: 'ic', gC: 'fr', b8: 'lbl', gG: 'iml', gH: 'imlf', gI: 'imlp', gJ: 'implw', gK: 'it', gO: 'i', d6: 'lnk', a7: 'nb', ee: 'notxt', g8: 'ol', g9: 'or', aK: 'oq', he: 'oh', en: 'pg', eo: 'p', hk: 'ppe', hw: 'ui', eJ: 'r', hy: 'sb', hz: 'sbx', hA: 'sby', hB: 'sbt', hH: 'e', hI: 'cap', hJ: 'sev', hR: 'sk', hX: 't', hY: 'tc', hZ: 'w8', h_: 'w2', h$: 'w9', h0: 'tj', cu: 'tja', h1: 'tl', h2: 'w3', h3: 'w5', h4: 'w4', h5: 'tr', h6: 'w6', h7: 'w1', h9: 'tun', e$: 'ts', aR: 'clr', ii: 'u', df: 'wc', e2: 'we', dg: 'wf', e3: 'wfp', dh: 'wrp'};
var $mdgriffith$elm_ui$Internal$Flag$overflow = $mdgriffith$elm_ui$Internal$Flag$flag(20);
var $mdgriffith$elm_ui$Element$clipX = A2($mdgriffith$elm_ui$Internal$Model$Class, $mdgriffith$elm_ui$Internal$Flag$overflow, $mdgriffith$elm_ui$Internal$Style$classes.fM);
var $mdgriffith$elm_ui$Element$clipY = A2($mdgriffith$elm_ui$Internal$Model$Class, $mdgriffith$elm_ui$Internal$Flag$overflow, $mdgriffith$elm_ui$Internal$Style$classes.fN);
...
var $mdgriffith$elm_ui$Internal$Model$columnClass = $mdgriffith$elm_ui$Internal$Style$classes.fm + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.u);
var $mdgriffith$elm_ui$Internal$Model$gridClass = $mdgriffith$elm_ui$Internal$Style$classes.fm + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.gs);
var $mdgriffith$elm_ui$Internal$Model$pageClass = $mdgriffith$elm_ui$Internal$Style$classes.fm + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.en);
var $mdgriffith$elm_ui$Internal$Model$paragraphClass = $mdgriffith$elm_ui$Internal$Style$classes.fm + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.eo);
var $mdgriffith$elm_ui$Internal$Model$rowClass = $mdgriffith$elm_ui$Internal$Style$classes.fm + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.eJ);
var $mdgriffith$elm_ui$Internal$Model$singleClass = $mdgriffith$elm_ui$Internal$Style$classes.fm + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.hH);
	...
	switch (style.$) {
		...
		case 8:
			...
			return 'grid-rows-' + (A2(
				$elm$core$String$join,
				'-',
				A2($elm$core$List$map, $mdgriffith$elm_ui$Internal$Model$lengthClassName, template.aC)) + ('-cols-' + (A2(
				$elm$core$String$join,
				'-',
				A2($elm$core$List$map, $mdgriffith$elm_ui$Internal$Model$lengthClassName, template.B)) + ('-space-x-' + ($mdgriffith$elm_ui$Internal$Model$lengthClassName(template.hK.a) + ('-space-y-' + $mdgriffith$elm_ui$Internal$Model$lengthClassName(template.hK.b)))))));
		case 9:
			var pos = style.a;
			return 'gp grid-pos-' + ($elm$core$String$fromInt(pos.eJ) + ('-' + ($elm$core$String$fromInt(pos.fO) + ('-' + ($elm$core$String$fromInt(pos.il) + ('-' + $elm$core$String$fromInt(pos.dQ)))))));
		case 11:
...
var $mdgriffith$elm_ui$Internal$Model$formatBoxShadow = function (shadow) {
	return A2(
		...
		A2(
			...
			_List_fromArray(
				[
					shadow.d_ ? $elm$core$Maybe$Just('inset') : $elm$core$Maybe$Nothing,
					$elm$core$Maybe$Just(
					$elm$core$String$fromFloat(shadow.as.a) + 'px'),
					$elm$core$Maybe$Just(
					$elm$core$String$fromFloat(shadow.as.b) + 'px'),
					$elm$core$Maybe$Just(
					$elm$core$String$fromFloat(shadow.fu) + 'px'),
					$elm$core$Maybe$Just(
					$elm$core$String$fromFloat(shadow.bd) + 'px'),
					$elm$core$Maybe$Just(
					$mdgriffith$elm_ui$Internal$Model$formatColor(shadow.fP))
				])));
};
...
var $mdgriffith$elm_ui$Internal$Model$renderFocusStyle = function (focus) {
	return _List_fromArray(
		[
								...
								$mdgriffith$elm_ui$Internal$Model$formatBoxShadow(
									{
										...
										as: A2(
											...
											A2($elm$core$Tuple$mapFirst, $elm$core$Basics$toFloat, shadow.as)),
										bd: shadow.bd
									}));
						},
						focus.hE),
			...
			A2(
			$mdgriffith$elm_ui$Internal$Model$Style,
			($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.fm) + ':focus .focusable, ') + (($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.fm) + '.focusable:focus, ') + ('.ui-slide-bar:focus + ' + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.fm) + ' .focusable-thumb'))),
			A2(
				$elm$core$List$filterMap,
				$elm$core$Basics$identity,
				_List_fromArray(
...
var $mdgriffith$elm_ui$Internal$Style$baseSheet = _List_fromArray(
	[
		...
		A2(
		$mdgriffith$elm_ui$Internal$Style$Class,
		$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.fm) + ':focus',
		_List_fromArray(
			[
		...
		_List_fromArray(
			[
				...
				$mdgriffith$elm_ui$Internal$Style$dot(
					_Utils_ap($mdgriffith$elm_ui$Internal$Style$classes.gx, $mdgriffith$elm_ui$Internal$Style$classes.aR)) + ':hover',
				_List_fromArray(
					[
						A2($mdgriffith$elm_ui$Internal$Style$Prop, 'opacity', '0')
					])),
				...
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot(
					_Utils_ap($mdgriffith$elm_ui$Internal$Style$classes.gx, $mdgriffith$elm_ui$Internal$Style$classes.aK)) + ':hover',
				_List_fromArray(
					[
						A2($mdgriffith$elm_ui$Internal$Style$Prop, 'opacity', '1')
					])),
				...
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot(
					_Utils_ap($mdgriffith$elm_ui$Internal$Style$classes.go, $mdgriffith$elm_ui$Internal$Style$classes.aR)) + ':focus',
				_List_fromArray(
					[
						A2($mdgriffith$elm_ui$Internal$Style$Prop, 'opacity', '0')
					])),
				...
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot(
					_Utils_ap($mdgriffith$elm_ui$Internal$Style$classes.go, $mdgriffith$elm_ui$Internal$Style$classes.aK)) + ':focus',
				_List_fromArray(
					[
						A2($mdgriffith$elm_ui$Internal$Style$Prop, 'opacity', '1')
					])),
				...
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot(
					_Utils_ap($mdgriffith$elm_ui$Internal$Style$classes.cy, $mdgriffith$elm_ui$Internal$Style$classes.aR)) + ':active',
				_List_fromArray(
					[
						A2($mdgriffith$elm_ui$Internal$Style$Prop, 'opacity', '0')
					])),
				...
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot(
					_Utils_ap($mdgriffith$elm_ui$Internal$Style$classes.cy, $mdgriffith$elm_ui$Internal$Style$classes.aK)) + ':active',
				_List_fromArray(
					[
						A2($mdgriffith$elm_ui$Internal$Style$Prop, 'opacity', '1')
					])),
				...
				_List_fromArray(
					[
						...
						A2(
						$mdgriffith$elm_ui$Internal$Style$Child,
						'u:first-of-type.' + $mdgriffith$elm_ui$Internal$Style$classes.fh,
						_List_fromArray(
							[
						...
						A2(
						$mdgriffith$elm_ui$Internal$Style$Child,
						'u:first-of-type.' + ($mdgriffith$elm_ui$Internal$Style$classes.fh + (' ~ s.' + $mdgriffith$elm_ui$Internal$Style$classes.ff)),
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'flex-grow', '0')
							])),
				...
				_List_fromArray(
					[
						...
						A2(
						$mdgriffith$elm_ui$Internal$Style$Child,
						's:first-of-type.' + $mdgriffith$elm_ui$Internal$Style$classes.fg,
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'flex-grow', '1'),
								A2(
						...
						A2(
						$mdgriffith$elm_ui$Internal$Style$Child,
						's:last-of-type.' + $mdgriffith$elm_ui$Internal$Style$classes.fg,
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'flex-grow', '1'),
								A2(
						...
						A2(
						$mdgriffith$elm_ui$Internal$Style$Child,
						's:only-of-type.' + $mdgriffith$elm_ui$Internal$Style$classes.fg,
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'flex-grow', '1'),
								A2(
						...
						A2(
						$mdgriffith$elm_ui$Internal$Style$Child,
						's:last-of-type.' + ($mdgriffith$elm_ui$Internal$Style$classes.fg + ' ~ u'),
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'flex-grow', '0')
							])),
						A2(
						$mdgriffith$elm_ui$Internal$Style$Child,
						'u:first-of-type.' + ($mdgriffith$elm_ui$Internal$Style$classes.fe + (' ~ s.' + $mdgriffith$elm_ui$Internal$Style$classes.fg)),
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'flex-grow', '0')
							])),
				...
				_List_fromArray(
					[
						...
						A2(
						$mdgriffith$elm_ui$Internal$Style$Child,
						$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.fm + ':first-child'),
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'margin', '0 !important')
							])),
						...
						$mdgriffith$elm_ui$Internal$Style$Child,
						$mdgriffith$elm_ui$Internal$Style$dot(
							$mdgriffith$elm_ui$Internal$Style$classes.fm + ($mdgriffith$elm_ui$Internal$Style$selfName(3) + (':first-child + .' + $mdgriffith$elm_ui$Internal$Style$classes.fm))),
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'margin', '0 !important')
							])),
						...
						$mdgriffith$elm_ui$Internal$Style$Child,
						$mdgriffith$elm_ui$Internal$Style$dot(
							$mdgriffith$elm_ui$Internal$Style$classes.fm + ($mdgriffith$elm_ui$Internal$Style$selfName(2) + (':first-child + .' + $mdgriffith$elm_ui$Internal$Style$classes.fm))),
						_List_fromArray(
							[
								A2($mdgriffith$elm_ui$Internal$Style$Prop, 'margin', '0 !important')
							])),
...
var $mdgriffith$elm_ui$Internal$Style$explainer = '\n.explain {\n    border: 6px solid rgb(174, 121, 15) !important;\n}\n.explain > .' + ($mdgriffith$elm_ui$Internal$Style$classes.fm + (' {\n    border: 4px dashed rgb(0, 151, 167) !important;\n}\n\n.ctr {\n    border: none !important;\n}\n.explain > .ctr > .' + ($mdgriffith$elm_ui$Internal$Style$classes.fm + ' {\n    border: 4px dashed rgb(0, 151, 167) !important;\n}\n\n')));
...
var $mdgriffith$elm_ui$Internal$Style$overrides = '@media screen and (-ms-high-contrast: active), (-ms-high-contrast: none) {' + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.fm) + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.eJ) + (' > ' + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.fm) + (' { flex-basis: auto !important; } ' + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.fm) + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.eJ) + (' > ' + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.fm) + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.bY) + (' { flex-basis: auto !important; }}' + ($mdgriffith$elm_ui$Internal$Style$inputTextReset + ($mdgriffith$elm_ui$Internal$Style$sliderReset + ($mdgriffith$elm_ui$Internal$Style$trackReset + ($mdgriffith$elm_ui$Internal$Style$thumbReset + $mdgriffith$elm_ui$Internal$Style$explainer)))))))))))))));
...
var $mdgriffith$elm_ui$Internal$Style$emptyIntermediate = F2(
	function (selector, closing) {
		return {av: closing, p: _List_Nil, U: _List_Nil, K: selector};
	});
					...
					case 3:
						...
						return _Utils_update(
							rendered,
							{
								p: A2(
									$elm$core$List$cons,
									{av: '\n}', p: _List_Nil, U: props, K: '@supports (' + (prop + (':' + (value + (') {' + parent.K))))},
									rendered.p)
							});
					case 5:
						var selector = rule.a;
...
var $mdgriffith$elm_ui$Internal$Style$renderCompact = function (styleClasses) {
	...
	var renderClass = function (rule) {
		...
		if (!_v2.b) {
			...
			return rule.K + ('{' + (renderValues(rule.U) + (rule.av + '}')));
		}
	};
				...
				case 0:
					...
					return _List_fromArray(
						[
							...
							('.' + ($mdgriffith$elm_ui$Internal$Style$classes.fm + (':focus ' + (selector + '-fs  {')))) + (renderedProps + '\n}'),
							(selector + '-fs:focus-within {') + (renderedProps + '\n}'),
							('.ui-slide-bar:focus + ' + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.fm) + (' .focusable-thumb' + (selector + '-fs {')))) + (renderedProps + '\n}')
						]);
				default:
					return _List_fromArray(
						[
	...
	function (options, rule, maybePseudo) {
		switch (rule.$) {
			...
			case 5:
				...
				var _class = '.' + cls;
				var any = '.' + $mdgriffith$elm_ui$Internal$Style$classes.fm;
				return $elm$core$List$concat(
					_List_fromArray(
						[
							A4(
			...
			case 9:
				...
				var msPosition = A2(
					...
					_List_fromArray(
						[
							...
							'-ms-grid-column: ' + ($elm$core$String$fromInt(position.fO) + ';'),
							'-ms-grid-column-span: ' + ($elm$core$String$fromInt(position.il) + ';')
						]));
				var modernPosition = A2(
					...
					_List_fromArray(
						[
							'grid-row: ' + ($elm$core$String$fromInt(position.eJ) + (' / ' + ($elm$core$String$fromInt(position.eJ + position.dQ) + ';'))),
							'grid-column: ' + ($elm$core$String$fromInt(position.fO) + (' / ' + ($elm$core$String$fromInt(position.fO + position.il) + ';')))
						]));
				var _class = '.grid-pos-' + ($elm$core$String$fromInt(position.eJ) + ('-' + ($elm$core$String$fromInt(position.fO) + ('-' + ($elm$core$String$fromInt(position.il) + ('-' + $elm$core$String$fromInt(position.dQ)))))));
	...
	function (name, modifier, _v0) {
		...
		return _List_fromArray(
			[
				...
				A2($mdgriffith$elm_ui$Internal$Model$bracket, '.' + (name + ('.' + (modifier + ('> .' + ($mdgriffith$elm_ui$Internal$Style$classes.hX + (', .' + (name + (' .' + (modifier + (' > .' + $mdgriffith$elm_ui$Internal$Style$classes.hX)))))))))), textAdjustment)
			]);
	});
	...
	function (fontToAdjust, _v0, otherFontName) {
		...
		return A2(
			...
			_Utils_ap(
				...
				A3($mdgriffith$elm_ui$Internal$Model$fontRule, name, $mdgriffith$elm_ui$Internal$Style$classes.gq, full)));
	});
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					A2(
					$mdgriffith$elm_ui$Internal$Model$bracket,
					'.' + (name + ('.' + ($mdgriffith$elm_ui$Internal$Style$classes.hI + ('> .' + ($mdgriffith$elm_ui$Internal$Style$classes.hX + (', .' + (name + (' .' + ($mdgriffith$elm_ui$Internal$Style$classes.hI + (' > .' + $mdgriffith$elm_ui$Internal$Style$classes.hX)))))))))),
					_List_fromArray(
						[
...
var $mdgriffith$elm_ui$Internal$Model$adjust = F3(
	function (size, height, vertical) {
		return {dQ: height / size, bd: size, e0: vertical};
	});
...
var $mdgriffith$elm_ui$Internal$Model$convertAdjustment = function (adjustment) {
	var lines = _List_fromArray(
		[adjustment.bn, adjustment.bl, adjustment.br, adjustment.by]);
	var lineHeight = 1.5;
	...
	return {
		bn: A3($mdgriffith$elm_ui$Internal$Model$adjust, capitalSize, ascender - newBaseline, capitalVertical),
		dK: A3($mdgriffith$elm_ui$Internal$Model$adjust, fullSize, ascender - descender, fullVertical)
	};
};
	...
	function (_static, opts, styles, children) {
		var dynamicStyleSheet = A2(
			...
			A3(
				...
				_Utils_Tuple2(
					$elm$core$Set$empty,
					$mdgriffith$elm_ui$Internal$Model$renderFocusStyle(opts.go)),
				styles).b);
		return _static ? A2(
			$elm$core$List$cons,
			_Utils_Tuple2(
					...
					return A3(
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										$elm$html$Html$Attributes$class($mdgriffith$elm_ui$Internal$Style$classes.fm + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.hH))
									]))
							]));
			}
		}();
					...
					_List_fromArray(
						[
							...
							A2(
								...
								' ',
								_List_fromArray(
									[$mdgriffith$elm_ui$Internal$Style$classes.fm, $mdgriffith$elm_ui$Internal$Style$classes.hH, $mdgriffith$elm_ui$Internal$Style$classes.bY, $mdgriffith$elm_ui$Internal$Style$classes.aw, $mdgriffith$elm_ui$Internal$Style$classes.fh])))
						]),
					_List_fromArray(
					...
					_List_fromArray(
						[
							...
							A2(
								...
								' ',
								_List_fromArray(
									[$mdgriffith$elm_ui$Internal$Style$classes.fm, $mdgriffith$elm_ui$Internal$Style$classes.hH, $mdgriffith$elm_ui$Internal$Style$classes.bY, $mdgriffith$elm_ui$Internal$Style$classes.aw, $mdgriffith$elm_ui$Internal$Style$classes.ff])))
						]),
					_List_fromArray(
						[html])) : html));
			case 1:
					...
					_List_fromArray(
						[
							...
							A2(
								...
								' ',
								_List_fromArray(
									[$mdgriffith$elm_ui$Internal$Style$classes.fm, $mdgriffith$elm_ui$Internal$Style$classes.hH, $mdgriffith$elm_ui$Internal$Style$classes.bY, $mdgriffith$elm_ui$Internal$Style$classes.fg])))
						]),
					_List_fromArray(
					...
					_List_fromArray(
						[
							...
							A2(
								...
								' ',
								_List_fromArray(
									[$mdgriffith$elm_ui$Internal$Style$classes.fm, $mdgriffith$elm_ui$Internal$Style$classes.hH, $mdgriffith$elm_ui$Internal$Style$classes.bY, $mdgriffith$elm_ui$Internal$Style$classes.fe])))
						]),
					_List_fromArray(
						[html])) : html));
			default:
...
var $mdgriffith$elm_ui$Internal$Model$textElementClasses = $mdgriffith$elm_ui$Internal$Style$classes.fm + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.hX + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.df + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.cW)))));
...
var $mdgriffith$elm_ui$Internal$Model$textElementFillClasses = $mdgriffith$elm_ui$Internal$Style$classes.fm + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.hX + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.dg + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.cX)))));
...
var $mdgriffith$elm_ui$Internal$Model$createElement = F3(
	function (context, children, rendered) {
					...
					case 1:
						...
						return _Utils_eq(context, $mdgriffith$elm_ui$Internal$Model$asParagraph) ? _Utils_Tuple2(
							A2(
								...
								htmls),
							$elm$core$List$isEmpty(existingStyles) ? styled.eU : _Utils_ap(styled.eU, existingStyles)) : _Utils_Tuple2(
							A2(
								$elm$core$List$cons,
								_Utils_Tuple2(
									key,
									A2(styled.gy, $mdgriffith$elm_ui$Internal$Model$NoStyleSheet, context)),
								htmls),
							$elm$core$List$isEmpty(existingStyles) ? styled.eU : _Utils_ap(styled.eU, existingStyles));
					case 2:
						var str = child.a;
						return _Utils_Tuple2(
							A2(
		...
		if (children.$ === 1) {
			...
			var keyed = _v1.a;
			var styles = _v1.b;
			var newStyles = $elm$core$List$isEmpty(styles) ? rendered.eU : _Utils_ap(rendered.eU, styles);
			if (!newStyles.b) {
				return $mdgriffith$elm_ui$Internal$Model$Unstyled(
					A5(
						...
						$mdgriffith$elm_ui$Internal$Model$Keyed(
							A3($mdgriffith$elm_ui$Internal$Model$addKeyedChildren, 'nearby-element-pls', keyed, rendered.fJ)),
				...
				return $mdgriffith$elm_ui$Internal$Model$Unstyled(
					A5(
						...
						$mdgriffith$elm_ui$Internal$Model$Unkeyed(
							A2($mdgriffith$elm_ui$Internal$Model$addChildren, unkeyed, rendered.fJ)),
						$mdgriffith$elm_ui$Internal$Model$NoStyleSheet));
			} else {
				var allStyles = newStyles;
				return $mdgriffith$elm_ui$Internal$Model$Styled(
						...
						switch (location) {
							case 0:
								return A2(
									...
									' ',
									_List_fromArray(
										[$mdgriffith$elm_ui$Internal$Style$classes.a7, $mdgriffith$elm_ui$Internal$Style$classes.hH, $mdgriffith$elm_ui$Internal$Style$classes.e8]));
							case 1:
								return A2(
									$elm$core$String$join,
									' ',
									_List_fromArray(
										[$mdgriffith$elm_ui$Internal$Style$classes.a7, $mdgriffith$elm_ui$Internal$Style$classes.hH, $mdgriffith$elm_ui$Internal$Style$classes.fq]));
							case 2:
								return A2(
									$elm$core$String$join,
									' ',
									_List_fromArray(
										[$mdgriffith$elm_ui$Internal$Style$classes.a7, $mdgriffith$elm_ui$Internal$Style$classes.hH, $mdgriffith$elm_ui$Internal$Style$classes.g9]));
							case 3:
								return A2(
									$elm$core$String$join,
									' ',
									_List_fromArray(
										[$mdgriffith$elm_ui$Internal$Style$classes.a7, $mdgriffith$elm_ui$Internal$Style$classes.hH, $mdgriffith$elm_ui$Internal$Style$classes.g8]));
							case 4:
								return A2(
									$elm$core$String$join,
									' ',
									_List_fromArray(
										[$mdgriffith$elm_ui$Internal$Style$classes.a7, $mdgriffith$elm_ui$Internal$Style$classes.hH, $mdgriffith$elm_ui$Internal$Style$classes.gC]));
							default:
								return A2(
									$elm$core$String$join,
									' ',
									_List_fromArray(
										[$mdgriffith$elm_ui$Internal$Style$classes.a7, $mdgriffith$elm_ui$Internal$Style$classes.hH, $mdgriffith$elm_ui$Internal$Style$classes.fp]));
						}
					}())
				]),
			_List_fromArray(
...
var $mdgriffith$elm_ui$Internal$Model$alignXName = function (align) {
	switch (align) {
		case 0:
			return $mdgriffith$elm_ui$Internal$Style$classes.cz + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.dk);
		case 2:
			return $mdgriffith$elm_ui$Internal$Style$classes.cz + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.dl);
		default:
			return $mdgriffith$elm_ui$Internal$Style$classes.cz + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.fc);
	}
};
var $mdgriffith$elm_ui$Internal$Model$alignYName = function (align) {
	switch (align) {
		case 0:
			return $mdgriffith$elm_ui$Internal$Style$classes.cA + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.fi);
		case 2:
			return $mdgriffith$elm_ui$Internal$Style$classes.cA + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.fb);
		default:
			...
			return (portion === 1) ? _Utils_Tuple3(
				...
				_List_fromArray(
					[
						...
						$mdgriffith$elm_ui$Internal$Style$classes.fm + ('.' + ($mdgriffith$elm_ui$Internal$Style$classes.u + (' > ' + $mdgriffith$elm_ui$Internal$Style$dot(
			...
			return (portion === 1) ? _Utils_Tuple3(
				...
				_List_fromArray(
					[
						A3(
						$mdgriffith$elm_ui$Internal$Model$Single,
						$mdgriffith$elm_ui$Internal$Style$classes.fm + ('.' + ($mdgriffith$elm_ui$Internal$Style$classes.eJ + (' > ' + $mdgriffith$elm_ui$Internal$Style$dot(
						...
						if (A2($mdgriffith$elm_ui$Internal$Flag$present, $mdgriffith$elm_ui$Internal$Flag$width, has)) {
							...
							switch (width.$) {
								...
								case 2:
									var portion = width.a;
									if (portion === 1) {
										var $temp$classes = classes + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.dg),
											$temp$node = node,
											$temp$has = A2(
											$mdgriffith$elm_ui$Internal$Flag$add,
					...
					case 9:
						...
						var newStyles = function () {
							switch (elem.$) {
								...
								default:
									var styled = elem.a;
									return _Utils_ap(styles, styled.eU);
							}
						}();
						var $temp$classes = classes,
							$temp$node = node,
...
var $mdgriffith$elm_ui$Internal$Model$focusDefaultStyle = {
	...
	hE: $elm$core$Maybe$Just(
		{
			fu: 0,
			fP: A4($mdgriffith$elm_ui$Internal$Model$Rgba, 155 / 255, 203 / 255, 1, 1),
...
var $mdgriffith$elm_ui$Internal$Model$optionsToRecord = function (options) {
				...
				case 0:
					...
					if (_v4.$ === 1) {
						return _Utils_update(
							record,
							{
								gx: $elm$core$Maybe$Just(hoverable)
							});
					} else {
						return record;
					}
				case 1:
					...
					if (_v5.$ === 1) {
						return _Utils_update(
							record,
							{
								go: $elm$core$Maybe$Just(focusStyle)
							});
					} else {
						return record;
					}
				default:
					...
					if (_v6.$ === 1) {
						return _Utils_update(
							record,
							{
								a6: $elm$core$Maybe$Just(renderMode)
							});
					} else {
						return record;
					}
	...
	return andFinally(
		A3(
			$elm$core$List$foldr,
			combine,
			{go: $elm$core$Maybe$Nothing, gx: $elm$core$Maybe$Nothing, a6: $elm$core$Maybe$Nothing},
			options));
};
			...
			A2(
				...
				$mdgriffith$elm_ui$Internal$Model$htmlClass(
					A2(
						...
						' ',
						_List_fromArray(
							[$mdgriffith$elm_ui$Internal$Style$classes.hw, $mdgriffith$elm_ui$Internal$Style$classes.fm, $mdgriffith$elm_ui$Internal$Style$classes.hH]))),
...
var $mdgriffith$elm_ui$Element$column = F2(
	function (attrs, children) {
		return A4(
			...
			A2(
				$elm$core$List$cons,
				$mdgriffith$elm_ui$Internal$Model$htmlClass($mdgriffith$elm_ui$Internal$Style$classes.fT + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.bp)),
				A2(
					$elm$core$List$cons,
...
var $mdgriffith$elm_ui$Element$pointer = A2($mdgriffith$elm_ui$Internal$Model$Class, $mdgriffith$elm_ui$Internal$Flag$cursor, $mdgriffith$elm_ui$Internal$Style$classes.fZ);
		...
		return A4(
			...
			A2(
				...
				A2(
					...
					A2(
						$elm$core$List$cons,
						$mdgriffith$elm_ui$Internal$Model$htmlClass($mdgriffith$elm_ui$Internal$Style$classes.b_ + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.aw + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.hB + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.ee)))))),
						A2(
							$elm$core$List$cons,
...
var $author$project$Main$clearTextButton = A2(
	...
	$author$project$Main$buttonStyle2,
	{
		...
		bz: $elm$core$Maybe$Just($author$project$Main$ClearText)
	});
...
var $author$project$Main$getTestButton = A2(
	...
	$author$project$Main$buttonStyle2,
	{
		...
		bz: $elm$core$Maybe$Just($author$project$Main$GetTest)
	});
...
var $author$project$Main$getTextButton = A2(
	...
	$author$project$Main$buttonStyle2,
	{
		...
		bz: $elm$core$Maybe$Just($author$project$Main$GetText)
	});
...
var $mdgriffith$elm_ui$Element$clip = A2($mdgriffith$elm_ui$Internal$Model$Class, $mdgriffith$elm_ui$Internal$Flag$overflow, $mdgriffith$elm_ui$Internal$Style$classes.fL);
...
var $mdgriffith$elm_ui$Element$Input$negateBox = function (box) {
	return {bm: -box.bm, bw: -box.bw, bE: -box.bE, bJ: -box.bJ};
};
					...
					case 5:
						...
						return _Utils_update(
							els,
							{
								...
								ai: A2($elm$core$List$cons, attr, els.ai),
								f: A2($elm$core$List$cons, attr, els.f),
								bi: A2($elm$core$List$cons, attr, els.bi)
							});
					case 7:
						var cls = attr.a;
						var _v2 = attr.b;
							...
							return _Utils_update(
								els,
								{
									...
									ai: A2(
										$elm$core$List$cons,
										newHeight,
										A2($elm$core$List$cons, newLineHeight, els.ai)),
	...
	function (isMultiline, stacked, attrs) {
		return function (redist) {
			return {
				...
				bi: $elm$core$List$reverse(redist.bi)
			};
		}(
			A3(
				$elm$core$List$foldl,
				A2($mdgriffith$elm_ui$Element$Input$redistributeOver, isMultiline, stacked),
				{R: _List_Nil, n: _List_Nil, ai: _List_Nil, f: _List_Nil, bi: _List_Nil},
				attrs));
	});
				...
				_Utils_ap(
					_List_fromArray(
						[
							...
							$mdgriffith$elm_ui$Internal$Model$htmlClass($mdgriffith$elm_ui$Internal$Style$classes.ee + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.hk)),
...
var $mdgriffith$elm_ui$Element$scrollbarY = A2($mdgriffith$elm_ui$Internal$Model$Class, $mdgriffith$elm_ui$Internal$Flag$overflow, $mdgriffith$elm_ui$Internal$Style$classes.hA);
...
var $mdgriffith$elm_ui$Element$Input$textHelper = F3(
	function (textInput, attrs, textOptions) {
		...
		var redistributed = A3(
			...
			_Utils_eq(textInput.x, $mdgriffith$elm_ui$Element$Input$TextArea),
			$mdgriffith$elm_ui$Element$Input$isStacked(textOptions.a3),
			withDefaults);
		...
		var inputElement = A4(
			...
			_Utils_ap(
				...
				_Utils_ap(
					_List_fromArray(
						[
							$mdgriffith$elm_ui$Element$Input$value(textOptions.hX),
							$mdgriffith$elm_ui$Internal$Model$Attr(
							$elm$html$Html$Events$onInput(textOptions.g7)),
							$mdgriffith$elm_ui$Element$Input$hiddenLabelAttribute(textOptions.a3),
									...
									function () {
										if (textOptions.hX === '') {
											...
											if (_v1.$ === 1) {
												...
												return _List_fromArray(
													[
														A3($mdgriffith$elm_ui$Element$Input$renderPlaceholder, place, _List_Nil, textOptions.hX === '')
													]);
											}
										} else {
											return _List_fromArray(
												[
													...
													A2(
														...
														_List_fromArray(
															[
																$elm$html$Html$Attributes$class($mdgriffith$elm_ui$Internal$Style$classes.gH)
															]),
														_List_fromArray(
															[
																$elm$html$Html$text(textOptions.hX + '\u00A0')
															])))
												]);
										}
									}()))
										...
										if (_v2.$ === 1) {
											...
											return _List_fromArray(
												[
													...
													A3($mdgriffith$elm_ui$Element$Input$renderPlaceholder, place, redistributed.R, textOptions.hX === ''))
												]);
										}
									}()
									])))),
		...
		return A3(
			...
			A2(
				$elm$core$List$cons,
				A2($mdgriffith$elm_ui$Internal$Model$Class, $mdgriffith$elm_ui$Internal$Flag$cursor, $mdgriffith$elm_ui$Internal$Style$classes.f_),
				A2(
					$elm$core$List$cons,
					$mdgriffith$elm_ui$Element$Input$isHiddenLabel(textOptions.a3) ? $mdgriffith$elm_ui$Internal$Model$NoAttribute : $mdgriffith$elm_ui$Element$spacing(5),
...
var $mdgriffith$elm_ui$Element$Input$multiline = F2(
	function (attrs, multi) {
		return A3(
			...
			{G: $elm$core$Maybe$Nothing, L: multi.hL, x: $mdgriffith$elm_ui$Element$Input$TextArea},
			attrs,
			{a3: multi.a3, g7: multi.g7, hm: multi.hm, hX: multi.hX});
	});
...
var $author$project$Main$inputText = function (model) {
	return A2(
		...
		{
			a3: $mdgriffith$elm_ui$Element$Input$labelHidden('Enter source text here'),
			g7: $author$project$Main$InputText,
...
var $folkertdev$elm_paragraph$Paragraph$startr = function (width) {
	return {
		aV: $folkertdev$elm_paragraph$SymmetricList$singleton(
			{gT: 0, bP: 0, il: 0}),
		gT: 1,
		il: width
	};
...
var $folkertdev$elm_paragraph$Paragraph$stepr = F3(
	function (options, w, state) {
		var tot_width = (w + 1) + state.il;
		var width_hd = function (p) {
			return (tot_width - p.il) - 1;
		};
		var waste = function (p) {
			return $folkertdev$elm_paragraph$Paragraph$single(p) ? 0 : (p.bP + A2(
				$elm$core$Basics$pow,
				options.ek - width_hd(p),
				2));
		};
		var tot_len = 1 + state.gT;
		var old_width_hd = function (p) {
			return (state.il - p.il) - 1;
		};
		var _new = function (p) {
			return $folkertdev$elm_paragraph$Paragraph$single(p) ? {gT: state.gT, bP: 0, il: state.il} : {
				gT: state.gT,
				bP: p.il + A2(
					$elm$core$Basics$pow,
		...
		var k = F2(
			function (p, q) {
				var wq0 = width_hd(q);
				var wp0 = width_hd(p);
				var rq0 = (options.d9 - wq0) + 1;
				return ($folkertdev$elm_paragraph$Paragraph$single(q) && (!p.il)) ? A2($elm$core$Basics$min, options.ek - wp0, rq0) : ($folkertdev$elm_paragraph$Paragraph$single(q) ? rq0 : A2(
					$elm$core$Basics$min,
					A2(
			...
			while (true) {
				...
				if (_v0.$ === 1) {
					...
					if (_v2.$ === 1) {
						return (_Utils_cmp(
							width_hd(q),
							options.d9) > 0) ? $folkertdev$elm_paragraph$SymmetricList$empty : ps_pq;
					} else {
						var p = _v2.a;
						if ((_Utils_cmp(
							waste(p),
		...
		var _v6 = $folkertdev$elm_paragraph$SymmetricList$last(state.aV);
		if (_v6.$ === 1) {
			return state;
		} else {
			var last = _v6.a;
			...
			return {aV: newCandidates, gT: tot_len, il: tot_width};
		}
	});
...
var $folkertdev$elm_paragraph$Paragraph$paragraph = F2(
	function (options, words) {
		var zs = A3(
			...
			A2($elm$core$List$map, options.eT, words));
		if (!zs.b) {
			return _List_Nil;
		} else {
			var first = zs.a;
...
var $author$project$Main$paragraphFormat = {d9: 80, ek: 70, eT: $elm$core$String$length};
var $author$project$Main$CYTMsg = function (a) {
	return {$: 6, a: a};
};
var $author$project$Main$initStateWithData = F2(
	function (k, data) {
		return {ft: 0, bu: k, hj: data, hC: '', il: 300};
	});
...
var $jxxcarlson$elm_graph$SimpleGraph$bMakeYLabels = F2(
	function (yMax, ga) {
		var n = $jxxcarlson$elm_graph$SimpleGraph$yTickmarks(ga.hd);
		var _v0 = !n;
		if (_v0) {
...
var $jxxcarlson$elm_graph$SimpleGraph$buildSVGTransformString = function (ga) {
	var _v0 = $jxxcarlson$elm_graph$SimpleGraph$scale(ga.hd);
	var kx = _v0.a;
	var ky = _v0.b;
...
var $jxxcarlson$elm_graph$SimpleGraph$bxTickmarks = function (ga) {
	var dx = $jxxcarlson$elm_graph$SimpleGraph$xTickmarks(ga.hd) * $jxxcarlson$elm_graph$SimpleGraph$deltaX(ga.hd);
...
var $jxxcarlson$elm_graph$SimpleGraph$barChartAsSVG = F2(
	function (ga, data) {
		...
		var barWidth = 0.8 * $jxxcarlson$elm_graph$SimpleGraph$deltaX(ga.hd);
		var gbar = function (_v1) {
			var x = _v1.a;
			var y = _v1.b;
			return A5(
				$jxxcarlson$elm_graph$SimpleGraph$barRect,
				$jxxcarlson$elm_graph$SimpleGraph$lineColor(ga.hd),
				barWidth,
				ga.cR,
				x,
				y);
		...
		var _v0 = A2(
			...
			$jxxcarlson$elm_graph$SimpleGraph$deltaX(ga.hd),
			data);
		var yMax = _v0.a;
...
var $mdgriffith$elm_ui$Element$Font$bold = A2($mdgriffith$elm_ui$Internal$Model$Class, $mdgriffith$elm_ui$Internal$Flag$fontWeight, $mdgriffith$elm_ui$Internal$Style$classes.fw);
...
var $author$project$Widget$Data$bargraph = F5(
	function (renderArgs, name, args, body, sm) {
		...
		var barGraphAttributes = {cR: graphHeight, cS: graphWidth, hd: options};
		return A2(
			$mdgriffith$elm_ui$Element$column,
			_List_Nil,
			_List_fromArray(
...
var $mdgriffith$elm_ui$Element$image = F2(
	function (attrs, _v0) {
		...
		return A4(
			...
			A2(
				$elm$core$List$cons,
				$mdgriffith$elm_ui$Internal$Model$htmlClass($mdgriffith$elm_ui$Internal$Style$classes.gA),
				attrs),
			$mdgriffith$elm_ui$Internal$Model$Unkeyed(
				_List_fromArray(
					[
...
var $author$project$Render$Elm$indentPadding = $mdgriffith$elm_ui$Element$paddingEach(
	{bm: 0, bw: 24, bE: 0, bJ: 0});
var $mdgriffith$elm_ui$Element$Font$italic = $mdgriffith$elm_ui$Internal$Model$htmlClass($mdgriffith$elm_ui$Internal$Style$classes.gO);
var $jxxcarlson$elm_graph$SimpleGraph$getDataWindow = function (pointList) {
	...
	return {ac: xMax, q: xMin, ad: yMax, y: yMin};
};
		...
		if ((!_v0.a) && (!_v0.b)) {
			...
			return _List_fromArray(
				[
					...
					_Utils_Tuple2(dw.ac, dw.y),
					_Utils_Tuple2(dw.ac, dw.ad),
					_Utils_Tuple2(dw.q, dw.ad),
					_Utils_Tuple2(dw.q, dw.y)
				]);
		}
	});
var $jxxcarlson$elm_graph$SimpleGraph$getScaleFactor = F2(
	function (dataWindow, gA) {
		var ky = gA.cR / (dataWindow.ad - dataWindow.y);
		var kx = gA.cS / (dataWindow.ac - dataWindow.q);
...
var $jxxcarlson$elm_graph$SimpleGraph$makeXLabel = F3(
	function (_v0, dw, x) {
		...
		var dx = $elm$core$String$fromFloat(kx * (x - dw.q));
		return A2(
			$elm$svg$Svg$text_,
			_List_fromArray(
				[
...
var $jxxcarlson$elm_graph$SimpleGraph$makeXTickMark = F3(
	function (ky, dw, x) {
		return _List_fromArray(
			[
				_Utils_Tuple2(x, dw.y),
				_Utils_Tuple2(x, dw.y - ($jxxcarlson$elm_graph$SimpleGraph$tickMarkLength / ky))
			]);
	});
...
var $jxxcarlson$elm_graph$SimpleGraph$makeYLabel = F3(
	function (_v0, dw, y) {
		...
		var label = $elm$core$String$fromFloat(
			A2($jxxcarlson$elm_graph$SimpleGraph$roundTo, 1, y));
		var dy = $elm$core$String$fromFloat((ky * (y - dw.y)) - 3);
		return A2(
			$elm$svg$Svg$text_,
			_List_fromArray(
				[
...
var $jxxcarlson$elm_graph$SimpleGraph$makeYTickMark = F3(
	function (kx, dw, y) {
		return _List_fromArray(
			[
				_Utils_Tuple2(dw.q, y),
				_Utils_Tuple2(dw.q - ($jxxcarlson$elm_graph$SimpleGraph$tickMarkLength / kx), y)
			]);
	});
...
var $jxxcarlson$elm_graph$SimpleGraph$lineChartAsSVGWithDataWindow = F3(
	function (dw, ga, data) {
		...
		var xLabels = A3(
			...
			scaleFactor,
			dw,
			$jxxcarlson$elm_graph$SimpleGraph$xTickmarks(ga.hd));
		var yLabels = A3(
			$jxxcarlson$elm_graph$SimpleGraph$makeYLabels,
			scaleFactor,
			dw,
			$jxxcarlson$elm_graph$SimpleGraph$yTickmarks(ga.hd));
		...
		var boundingBox_ = renderPlain(
			A2($jxxcarlson$elm_graph$SimpleGraph$boundingBox, ga.hd, dw));
		var abscissa = renderPlain(
			_List_fromArray(
				[
					_Utils_Tuple2(dw.q, 0),
					_Utils_Tuple2(dw.ac, 0)
				]));
		return A2(
			$elm$svg$Svg$g,
			_List_fromArray(
...
var $author$project$Widget$Data$linegraph = F5(
	function (renderArgs, name, args, body, sm) {
		...
		var lineGraphAttributes = {cR: graphHeight, cS: graphWidth, hd: options};
		return A2(
			$mdgriffith$elm_ui$Element$column,
			_List_Nil,
			_List_fromArray(
					...
					A2(
						...
						A2(
							...
							A2(
								...
								A2(
									$elm$core$List$cons,
									$mdgriffith$elm_ui$Internal$Model$htmlClass($mdgriffith$elm_ui$Internal$Style$classes.b_ + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.aw + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.d6)))),
									attrs)))))),
			$mdgriffith$elm_ui$Internal$Model$Unkeyed(
				_List_fromArray(
					[label])));
		...
		return $mdgriffith$elm_ui$Element$html(
			A3(
				...
				_List_fromArray(
					[
						_Utils_Tuple2(
						$elm$core$String$fromInt(renderArgs.bu),
						A4($author$project$Render$Elm$mathText_, displayMode, renderArgs.hC, content, meta))
					])));
	});
...
var $jxxcarlson$elm_graph$SimpleGraph$scatterPlotAsSVG = F2(
	function (ga, data) {
		...
		var xScaleFactor = ga.cS / (dw.ac - dw.q);
		var yScaleFactor = ga.cR / (dw.ad - dw.y);
		var rescaledData = A2(
			$elm$core$List$map,
			function (_v1) {
				var x = _v1.a;
		...
		var boundingBox_ = renderPlain(
			A2(
				...
				_Utils_update(
					dw,
					{ac: dw.ac + (diameter / xScaleFactor), ad: dw.ad + (diameter / yScaleFactor)})));
		return A2(
			$elm$svg$Svg$g,
			_List_fromArray(
				[transformer]),
...
var $author$project$Widget$Data$scatterplot = F5(
	function (renderArgs, name, args, body, sm) {
		...
		var scatterPlotAttributes = {cR: graphHeight, cS: graphWidth, hd: options};
		return A2(
			$mdgriffith$elm_ui$Element$column,
			_List_Nil,
			_List_fromArray(
...
var $author$project$Render$Elm$renderTocItem = function (tocItem) {
	return A2(
		...
		_List_fromArray(
			[
				$mdgriffith$elm_ui$Element$Font$color($author$project$Render$Elm$linkColor),
				$author$project$Render$Elm$tocPadding(tocItem.d4)
			]),
		{
			a3: $mdgriffith$elm_ui$Element$text(tocItem.a3 + (' ' + tocItem.ec)),
			de: '#' + $author$project$Render$Utility$slug(tocItem.ec)
		});
};
		...
		return A2(
			...
			A2(
				...
				A2(
					...
					$elm$core$List$reverse(renderArgs.hj.bf))));
	});
...
var $author$project$Parser$Data$defaultConfig = {dp: '#00c', dT: '#fAA', eC: '#a00'};
...
var $author$project$Parser$Document$init = F2(
	function (generation, strList) {
		return {
			...
			M: 0,
			af: 0,
			f$: $author$project$Parser$Data$init($author$project$Parser$Data$defaultConfig),
			bu: generation,
			ai: strList,
			ap: 0,
			P: _List_Nil
...
var $author$project$Parser$Metadata$dummy = {ft: 0, bu: 0, a3: '', gT: 0, as: 0};
var $author$project$Parser$RecoveryData$problemWithElement = {
	...
	hh: A4(
		...
		A2(
			...
			$elm$core$Maybe$Nothing),
		$elm$core$Maybe$Nothing),
	hr: $author$project$Parser$Error$ExpectingRightBracket,
	h8: 1
};
		...
		return A2(
			...
			function (r) {
				return _Utils_update(
					r,
					{
						hh: A2($author$project$Parser$RecoveryData$setSourceMap, newSourceMap, r.hh)
					});
			},
...
var $author$project$Parser$Driver$handlePipeError = F4(
	function (tc_, mFirstError, errorColumn, mRecoveryData) {
		var textLines = $elm$core$String$lines(tc_.hX);
		var errorRow = A2(
			$elm$core$Maybe$withDefault,
			0,
			A2(
		...
		return {
			...
			c3: $author$project$Parser$Driver$parse__(
...
var $author$project$Parser$Driver$handleRightBracketError = F4(
	function (tc_, mFirstError, errorColumn, mRecoveryData) {
		...
		return {
			...
			as: A3($author$project$Parser$Driver$newOffset, tc_, errorColumn, mRecoveryData),
			hg: $elm$core$Maybe$Nothing,
			c3: A2($elm$core$List$drop, 1, tc_.c3),
			aO: _List_Nil,
			hX: newText
		};
	});
var $author$project$Parser$Driver$makeNewText = F3(
	function (tc_, errorColumn_, mRecoveryData) {
		if (!mRecoveryData.$) {
			...
			return A2($elm$core$String$dropLeft, rd.h8, tc_.hX);
		} else {
			return A2($elm$core$String$dropLeft, errorColumn_, tc_.hX);
		}
	});
var $author$project$Parser$Driver$newParsed = F3(
	function (tc_, lxError_, mRecoveryData) {
		if (!mRecoveryData.$) {
			var rd = mRecoveryData.a;
			return A2($elm$core$List$cons, rd.hh, tc_.c3);
		} else {
			return A2($elm$core$List$cons, lxError_, tc_.c3);
		}
	});
var $author$project$Parser$Driver$newStack = F3(
	function (tc_, errorText_, mRecoveryData) {
		if (!mRecoveryData.$) {
			return _Utils_eq(
				$elm$core$List$head(tc_.aO),
				$elm$core$Maybe$Just('highlight')) ? tc_.aO : A2($elm$core$List$cons, 'highlight', tc_.aO);
		} else {
			return A2(
				$elm$core$List$cons,
				errorText_,
				A2($elm$core$List$cons, 'highlight', tc_.aO));
		}
	});
var $author$project$Parser$Driver$unhandledError = F6(
	function (tc_, mFirstError, errorColumn, mRecoveryData, lxError, errorText) {
		return {
			...
			as: A3($author$project$Parser$Driver$newOffset, tc_, errorColumn, mRecoveryData),
			hg: $elm$core$Maybe$Nothing,
			c3: A3($author$project$Parser$Driver$newParsed, tc_, lxError, mRecoveryData),
			aO: A3($author$project$Parser$Driver$newStack, tc_, errorText, mRecoveryData),
			hX: A3($author$project$Parser$Driver$makeNewText, tc_, errorColumn, mRecoveryData)
		};
	});
	...
	function (tc_, e) {
		...
		var errorColumn = A2(
			...
			A2(
				...
				mFirstError));
		var errorText = A2($elm$core$String$left, errorColumn, tc_.hX);
		var lxError = A4(
			$author$project$Parser$Element$Element,
			'Error',
			_List_Nil,
			...
			$elm$core$Maybe$Just(
				{ft: tc_.aE, bu: tc_.bu, a3: '', gT: errorColumn, as: tc_.as + errorColumn}));
		switch (problem.$) {
			case 2:
...
var $author$project$Parser$Driver$packet = {
	gr: $author$project$Parser$Getters$getSource,
	gt: $elm$core$Maybe$Just($author$project$Parser$Driver$handleError),
	gw: $elm$core$Maybe$Nothing,
	gD: $author$project$Parser$Driver$incrementOffset,
	hi: $author$project$Parser$Element$parser
};
var $author$project$Parser$TextCursor$init = F4(
	function (generation, blockIndex, data, text) {
		return {fs: '', aE: blockIndex, C: 0, f$: data, bu: generation, as: 0, hg: $elm$core$Maybe$Nothing, c3: _List_Nil, aO: _List_Nil, hX: text};
	});
...
var $author$project$Parser$Data$sectionLabel = function (data) {
	return A2(
		...
		A2(
			...
			A2($elm$core$Dict$get, 'section', data.bO)));
};
...
var $author$project$Parser$Loop$newExpr = F3(
	function (packet, tc_, expr) {
		var _v0 = $elm$core$List$head(tc_.aO);
		if ((!_v0.$) && (_v0.a === 'highlight')) {
			var _v1 = packet.gw;
			if (_v1.$ === 1) {
				return A2(packet.gD, tc_.as, expr);
			} else {
				var hl = _v1.a;
				return A2(
					packet.gD,
...
var $author$project$Vector$increment = F2(
	function (k, v) {
		return ((k < 0) || (_Utils_cmp(k, v.bd) > -1)) ? v : A2(
...
var $author$project$Parser$Data$handleSection = F3(
	function (name, element, data) {
		...
		if (level_.$ === 1) {
			...
			var newVectorCounters = A3($elm$core$Dict$update, 'section', maybeInc, data.bO);
			var newData = _Utils_update(
				data,
				{bO: newVectorCounters});
			var tocItem = {
				a3: $author$project$Parser$Data$sectionLabel(newData),
				d4: level - 1,
				ec: $author$project$Parser$Data$getText(element)
			};
			return _Utils_update(
				newData,
				{
					bf: A2($elm$core$List$cons, tocItem, data.bf)
				});
		}
	});
		...
		if ((tc.hX === '') || (tc.C > 100)) {
			return $author$project$Parser$Tool$Done(
				_Utils_update(
					tc,
					{
						c3: $elm$core$List$reverse(tc.c3)
					}));
		} else {
			var _v0 = A2(
				$elm$parser$Parser$Advanced$run,
				A2(packet.hi, tc.bu, tc.aE),
				tc.hX);
			if (!_v0.$) {
				var expr = _v0.a;
				...
				var data = A2($author$project$Parser$Data$update, parsand, tc.f$);
				return $author$project$Parser$Tool$Loop(
					_Utils_update(
						tc,
						{
							fs: _Utils_ap(
								tc.fs,
								A2($elm$core$String$left, sourceMapLength, tc.hX)),
							C: tc.C + 1,
							f$: data,
							as: tc.as + sourceMapLength,
							hg: $elm$core$Maybe$Just(parsand),
							c3: A2(
								$elm$core$List$cons,
								A2($author$project$Parser$Data$labelElement, data, parsand),
								tc.c3),
							hX: A2($elm$core$String$dropLeft, sourceMapLength, tc.hX)
						}));
			} else {
				var e = _v0.a;
				var _v1 = packet.gt;
...
var $author$project$Parser$Document$updateData = function (tc) {
	...
	if (_v0.$ === 1) {
		...
		var parsand = _v0.a;
		return A2($author$project$Parser$Data$update, parsand, tc.f$);
	}
};
var $author$project$Parser$Document$pushBlock_ = F2(
	function (line, state) {
		...
		var tc = A4($author$project$Parser$Driver$parseLoop, state.bu, state.ap, state.f$, str);
		return _Utils_update(
			state,
			{
				o: _List_Nil,
				M: 0,
				af: 0,
				f$: $author$project$Parser$Document$updateData(tc),
				ap: state.ap + $author$project$Parser$Document$countLines(state.o),
				P: A2($elm$core$List$cons, tc, state.P)
			});
	});
var $author$project$Parser$Document$addToBlockContents = F2(
	function (currentLine_, state) {
		...
		var newBlockLevel = state.M + deltaBlockLevel;
...
var $elm$parser$Parser$DeadEnd = F3(
	function (row, col, problem) {
		return {fO: col, hr: problem, eJ: row};
	});
var $elm$parser$Parser$problemToDeadEnd = function (p) {
	return A3($elm$parser$Parser$DeadEnd, p.eJ, p.fO, p.hr);
};
...
var $author$project$Parser$Document$flush = function (state) {
	...
	var newState = function () {
		if (input === '') {
			...
			var tc_ = A4($author$project$Parser$Driver$parseLoop, state.bu, state.ap, state.f$, input);
...
var $author$project$Parser$Document$popBlockStack = F2(
	function (currentLine_, state) {
		var newBlockLevel = state.M + $author$project$Parser$Document$differentialBlockLevel(currentLine_);
		if (!newBlockLevel) {
			var input_ = A2(
				$elm$core$String$join,
				'\n',
			...
			var tc_ = A4($author$project$Parser$Driver$parseLoop, state.bu, state.ap, state.f$, input_);
			var tc = _Utils_update(
				tc_,
				{hX: input_});
			return _Utils_update(
				state,
				{
					...
					af: 0,
					f$: $author$project$Parser$Document$updateData(tc),
					ap: state.ap + (2 + $elm$core$List$length(state.o)),
					P: A2($elm$core$List$cons, tc, state.P)
				});
		} else {
			return _Utils_update(
...
var $author$project$Parser$Document$nextState = function (state_) {
	var _v0 = $elm$core$List$head(state_.ai);
	if (_v0.$ === 1) {
		...
		var state = _Utils_update(
			state_,
			{
				ai: A2($elm$core$List$drop, 1, state_.ai)
			});
		var _v1 = _Utils_Tuple2(
			state.af,
...
var $author$project$Main$render = F2(
	function (k, str) {
		var state = A2(
			...
			k,
			$elm$core$String$lines(str));
		var newState = A2($author$project$Main$initStateWithData, k, state.f$);
		return A2(
...
var $author$project$Main$outputDisplay_ = function (model) {
	return A2(
		...
		(model.a6 === 1) ? _List_fromArray(
			[
				A2($author$project$Main$render, model.C, model.ai)
			]) : A2(
			$elm$core$List$map,
			$mdgriffith$elm_ui$Element$text,
			A2($folkertdev$elm_paragraph$Paragraph$lines, $author$project$Main$paragraphFormat, model.aN)));
};
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$author$project$Main$rawModeButton(model.a6),
						$author$project$Main$renderedModeButton(model.a6),
						$mdgriffith$elm_ui$Element$text(
						'generation: ' + $elm$core$String$fromInt(model.C)),
						$author$project$Main$wordCountElement(model.ai)
					])),
...
var $author$project$Main$parsed = function (model) {
	var _v0 = A2(
		...
		A3($author$project$Parser$Element$parseList, model.C, 0, model.ai));
	if (_v0.$ === 1) {
...
var $author$project$Main$noFocus = {fo: $elm$core$Maybe$Nothing, fx: $elm$core$Maybe$Nothing, hE: $elm$core$Maybe$Nothing};
...
var $author$project$Main$main = $elm$browser$Browser$element(
	{gF: $author$project$Main$init, hT: $author$project$Main$subscriptions, ij: $author$project$Main$update, ik: $author$project$Main$view});
