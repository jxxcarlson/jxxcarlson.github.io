...
var $author$project$Parser$Driver$packet = {
	getLength: $author$project$Parser$AST$length,
	handleError: $elm$core$Maybe$Nothing,
	parser: function (str) {
...
var $author$project$Parser$AST$StackError = F4(
	function (a, b, c, d) {
		return {$: 'StackError', a: a, b: b, c: c, d: d};
	});
			...
			_Utils_ap(
				_List_fromArray(
					[
						...
						A2($author$project$Parser$AST$Text, ' unmatched ' + (top.expect.beginSymbol + ' '), $author$project$Parser$MetaData$dummy),
...
var $author$project$Parser$TextCursor$handleHeadings = F3(
	function (tc, top, parsed_) {
		return (top.expect.beginSymbol === '#') ? _Utils_ap(
			$elm$core$List$reverse(tc.complete),
			_List_fromArray(
				[
					A3(
					...
					A2(
						...
						$author$project$Parser$MetaData$dummy),
					$author$project$Parser$MetaData$dummy)
				])) : ((top.expect.beginSymbol === '##') ? _Utils_ap(
			$elm$core$List$reverse(tc.complete),
			_List_fromArray(
				[
					A3(
...
var $author$project$Parser$TextCursor$commit = function (tc) {
	return function (tc2) {
		return _Utils_update(
			tc2,
			{
				complete: $elm$core$List$reverse(tc2.complete)
			});
	}(
	...
	if (!_v0.b) {
		...
		var complete_ = function () {
			...
			if (_v1.$ === 'Nothing') {
				...
				if (A2($elm$core$String$left, 1, top.expect.beginSymbol) === '#') {
					return A3($author$project$Parser$TextCursor$handleHeadings, tc, top, parsed_);
				} else {
					if (top.expect.beginSymbol === ':') {
						...
						var errorMessage = A4(
							...
							top.scanPoint,
							tc.scanPoint,
							'((unknown delimiter ' + (top.expect.beginSymbol + (' at position ' + ($elm$core$String$fromInt(top.scanPoint) + '))'))),
							A3($elm$core$String$slice, top.scanPoint, tc.scanPoint, tc.source));
						return _Utils_ap(
							$elm$core$List$reverse(tc.complete),
							_List_fromArray(
								[errorMessage]));
...
var $author$project$Parser$TextCursor$canPopPrecondition = F3(
	function (configuration, tc, prefix) {
		canPopPrecondition:
		while (true) {
			var isEndSymbol = A3($author$project$Parser$Config$isEndSymbol, configuration, tc.scanPoint, prefix);
			if (isEndSymbol) {
				return true;
			} else {
...
var $author$project$Parser$TextCursor$canPush = F3(
	function (configuration, tc, prefix) {
		return A3($author$project$Parser$Config$isBeginSymbol, configuration, tc.scanPoint, prefix);
	});
...
var $author$project$Parser$TextCursor$handleText = F3(
	function (parse, stackTop, tc) {
		...
		if (_v0.$ === 'Nothing') {
			return _Utils_update(
				tc,
				{count: tc.count + 1, scanPoint: tc.scanPoint + 1});
		} else {
			var stackTop_ = _v0.a;
			var _v1 = A2(
				$elm$core$Maybe$withDefault,
...
var $author$project$Parser$TextCursor$pop = F2(
	function (parse, tc) {
		...
		if (_v0.$ === 'Nothing') {
			return _Utils_update(
				tc,
				{count: tc.count + 1, scanPoint: tc.scanPoint + 1, scannerType: $author$project$Parser$TextCursor$NormalScan});
		} else {
			var stackTop = _v0.a;
		...
		return _Utils_update(
			tc,
			{
				complete: _Utils_eq(tc.stack, _List_Nil) ? ((tc.text === '') ? _List_Nil : A2(
					$elm$core$List$cons,
					A2($author$project$Parser$AST$Text, tc.text, $author$project$Parser$MetaData$dummy),
					_Utils_ap(tc.parsed, tc.complete))) : tc.complete,
				count: tc.count + 1,
				...
				stack: function () {
					...
					if (_v0.$ === 'Nothing') {
						return A2(
							$elm$core$List$cons,
							{content: '', count: tc.count, expect: expectation, precedingText: _List_Nil, scanPoint: tc.scanPoint},
							tc.stack);
					} else {
						var stackTop = _v0.a;
						return A2(
			...
			if (A3($author$project$Parser$TextCursor$canPush, $author$project$Parser$Loop$configuration, tc, prefix)) {
				...
				if (_v0.$ === 'Nothing') {
					return $author$project$Utility$ParserTools$Done(
						_Utils_update(
							tc,
							{message: 'Unexpected error: no corresponding expectation'}));
				} else {
				...
				return $author$project$Utility$ParserTools$Done(
					_Utils_update(
						tc,
						{message: 'Could neither push nor pop.  What the heck?'}));
			}
		}
	});
...
var $author$project$Parser$Print$printCaret = $author$project$Utility$Console$bgRed(' ^ ');
...
var $author$project$Parser$Print$printPreceding = function (cursor) {
				...
				_Utils_ap(
					...
					_Utils_ap(
						...
						_Utils_ap(
							...
							_Utils_ap(
								$author$project$Parser$Print$printPreceding(cursor),
								_Utils_ap(
...
var $author$project$Parser$Loop$nextCursor = F2(
	function (packet, cursor) {
		var _v0 = A2(
			...
			'');
		if (_Utils_cmp(cursor.scanPoint, cursor.length) > -1) {
			...
			var chompedText = function () {
				...
				if (_v2.$ === 'NormalScan') {
					return A3($author$project$Parser$Loop$advance, $author$project$Parser$Loop$configuration, cursor.scanPoint, textToProcess);
				} else {
					var c = _v2.a;
			...
			if ((chompedText.finish - chompedText.start) > 0) {
				...
				if (_v1.$ === 'Nothing') {
					...
					return A3(
						...
						packet,
						A2($author$project$Utility$ParserTools$prefixWith, c, textToProcess).content,
						cursor);
				}
			}
		}
		...
		var headerPadding = $mdgriffith$elm_ui$Element$paddingEach(
			{
				...
				left: 0,
				right: 0,
				top: $elm$core$Basics$round(12 / factor)
			});
		return A2(
...
var $author$project$Render$Elm$renderMath = F4(
	function (renderArgs, name, args, body) {
		...
		if (_v0.$ === 'Just') {
			var content = _v0.a;
			return A3($author$project$Render$Elm$mathText, renderArgs, $author$project$Render$Elm$InlineMathMode, content);
		} else {
			return A2(
				$mdgriffith$elm_ui$Element$el,
				_List_fromArray(
	...
	return _Utils_Tuple2(
		{
			...
			renderedText: A2($author$project$Main$render, 0, $author$project$Main$initialText),
